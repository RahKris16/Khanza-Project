...
//        this.setCursor(Cursor.getDefaultCursor());
        ...
        if (NoRMPasien.getText().equals("")) {
            JOptionPane.showMessageDialog(rootPane, "No Rekam Medis kosong ");
            NoRMPasien.setText("");
        } else if (Sequel.cariInteger("SELECT count(no_rkm_medis) FROM pasien WHERE no_rkm_medis = '"+NoRMPasien.getText().trim()+"'") == 0 && Sequel.cariInteger("SELECT COUNT(p.no_ktp) FROM pasien p WHERE p.no_ktp= '"+NoRMPasien.getText().trim()+"'") == 0) {
            JOptionPane.showMessageDialog(rootPane, "Data pasien tidak ditemukan ");
            NoRMPasien.setText("");
        } else if (Sequel.cariInteger("SELECT count(kd_pj) FROM booking_registrasi WHERE no_rkm_medis = '"+NoRMPasien.getText().trim()+"' AND kd_pj!='A09' AND tanggal_periksa=current_date()") > 0 || Sequel.cariInteger("SELECT count(br.kd_pj) FROM booking_registrasi br inner join pasien p on p.no_rkm_medis=br.no_rkm_medis WHERE p.no_ktp='"+NoRMPasien.getText().trim()+"' AND br.kd_pj!='A09' AND br.tanggal_periksa=current_date()") > 0) {
            JOptionPane.showMessageDialog(rootPane, "Maaf, Anda menggunakan Asuransi. \n Untuk pasien BPJS, Silahkan cekin menggunakan menu BPJS.\n Untuk Asuransi silahkan menuju loket Admisi ");
            NoRMPasien.setText("");
        } else if (Sequel.cariInteger("SELECT count(no_rkm_medis) FROM booking_registrasi WHERE no_rkm_medis = '"+NoRMPasien.getText().trim()+"' AND tanggal_periksa=current_date()") < 1 && Sequel.cariInteger("SELECT count(p.no_ktp) FROM booking_registrasi br inner join pasien p on p.no_rkm_medis=br.no_rkm_medis WHERE p.no_ktp='"+NoRMPasien.getText().trim()+"' AND br.tanggal_periksa =CURRENT_DATE()") < 1) {
            JOptionPane.showMessageDialog(rootPane, "Data Booking tidak ditemukan ");
            NoRMPasien.setText("");
        } else if (Sequel.cariInteger("SELECT count(no_rkm_medis) FROM booking_registrasi WHERE status='Terdaftar' and no_rkm_medis = '"+NoRMPasien.getText().trim()+"' AND tanggal_periksa=current_date()") > 0 || Sequel.cariInteger("SELECT count(p.no_ktp) FROM booking_registrasi br inner join pasien p on p.no_rkm_medis=br.no_rkm_medis WHERE br.status='Terdaftar' and p.no_ktp= '"+NoRMPasien.getText().trim()+"' AND br.tanggal_periksa=current_date()") > 0) {
            JOptionPane.showMessageDialog(rootPane, "Data Booking telah terdaftar ");
            NoRMPasien.setText("");
        } else if (Sequel.cariInteger("SELECT count(no_rkm_medis) FROM booking_registrasi WHERE status='Batal' and no_rkm_medis = '"+NoRMPasien.getText().trim()+"' AND tanggal_periksa=current_date()") > 0 || Sequel.cariInteger("SELECT count(p.no_ktp) FROM booking_registrasi br inner join pasien p on p.no_rkm_medis=br.no_rkm_medis WHERE br.status='Batal' and p.no_ktp= '"+NoRMPasien.getText().trim()+"' AND br.tanggal_periksa=current_date()") > 0) {
            JOptionPane.showMessageDialog(rootPane, "Data Booking telah dibatalkan ");
            NoRMPasien.setText("");
//        } else if (GeneralConsentSatuSehat(NoRMPasien.getText()) == false) {
//            int i = JOptionPane.showConfirmDialog(rootPane, "Anda perlu menyetujui Inform Consent terbaru tentang Platform SATUSEHAT. \n Apakah anda menyetujui? \n", "Konfirmasi", JOptionPane.YES_NO_OPTION);
...
//                            MnCetakBarcodeActionPerformed(NoRawat.getText());
//                        }
...
//                        Valid.webSocket(jsonData, "encounterkirim");
    ...
    private void MnCetakBarcodeActionPerformed(String norawat) {
        ...
        param.put("emailrs", Sequel.cariIsi("select email from setting"));
        param.put("logo", Sequel.cariGambar("select logo from setting"));
        Valid.MyReportqryabdul("rptBarcodeRM.jasper","report","::[ Label Rekam Medis ]::","select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
