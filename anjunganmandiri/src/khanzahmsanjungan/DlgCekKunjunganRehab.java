 ...
 * @author Kode
 */
public class DlgCekKunjunganRehab extends javax.swing.JDialog {
     ...
     * @param id
     */
    public DlgCekKunjunganRehab(java.awt.Frame parent, boolean id) {
        super(parent, id);
    ...
    private void initComponents() {
        ...
        TanggalRujukKeluar.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-05-2026" }));
        ...
        TanggalKunjungRujukan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-05-2026" }));
            ...
            if (!NoRMPasien.getText().equals("")) {
                if (Sequel.cariInteger("select count(referensi_mobilejkn_bpjs.norm) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.norm='" + NoRMPasien.getText() + "' and referensi_mobilejkn_bpjs.tanggalperiksa=current_date") > 0) {
                    ...
                    if (Sequel.cariInteger("select count(pasien.no_peserta) from pasien where pasien.no_peserta='" + NoRMPasien.getText() + "'") == 1) {
                        ...
                        DlgRegistrasiSEPRehab form = new DlgRegistrasiSEPRehab(null, true);
...
//        WindowMenu.setVisible(true);
        if (!NoRMPasien.getText().equals("")) {
            if (Sequel.cariInteger("select count(referensi_mobilejkn_bpjs.norm) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.norm='" + NoRMPasien.getText() + "' and referensi_mobilejkn_bpjs.tanggalperiksa=current_date") > 0) {
                ...
                if (Sequel.cariInteger("select count(pasien.no_peserta) from pasien where pasien.no_peserta='" + NoRMPasien.getText() + "'") == 1) {
                    if(Sequel.cariInteger("select count(p.no_peserta) from reg_periksa rp "
                            + "inner join pasien p on p.no_rkm_medis=rp.no_rkm_medis "
                            + "where rp.tgl_registrasi between CURDATE() and CURDATE() and rp.kd_poli='IGD' and kd_pj='BPJ' and p.no_peserta='" + NoRMPasien.getText() + "'")>0){
    ...
    //                    form.tampilfingerprint(NoRMPasien.getText());
                    ...
                    if(Sequel.cariInteger("select count(no_rkm_medis) from reg_periksa where tgl_registrasi between CURDATE() and CURDATE() and kd_poli='IGD' and kd_pj='BPJ' and no_rkm_medis='" + NoRMPasien.getText() + "'")>0){
                        JOptionPane.showMessageDialog(rootPane, "Maaf, SEP tidak dapat dibuat. Pasien sudah pernah berkunjung ke rumah sakit");
                    }else{
                            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                            DlgRegistrasiSEPRehab form = new DlgRegistrasiSEPRehab(null, true);
        ...
        //                    form.tampilfingerprint(Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_rkm_medis='" + NoRMPasien.getText() + "'"));
                    ...
                    if(Sequel.cariInteger("select count(p.no_ktp) from reg_periksa rp "
                            + "inner join pasien p on p.no_rkm_medis=rp.no_rkm_medis "
                            + "where rp.tgl_registrasi between CURDATE() and CURDATE() and rp.kd_poli='IGD' and kd_pj='BPJ' and p.no_ktp='" + NoRMPasien.getText() + "'")>0){
                        JOptionPane.showMessageDialog(rootPane, "Maaf, SEP tidak dapat dibuat. Pasien sudah pernah berkunjung ke rumah sakit");
                    }else{
                        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                        DlgRegistrasiSEPRehab form = new DlgRegistrasiSEPRehab(null, true);
...
//                Notifications.getInstance().show(Notifications.Type.WARNING, Notifications.Location.TOP_CENTER, "Data surat kontrol tidak ditemukan!");
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCekKunjunganRehab dialog = new DlgCekKunjunganRehab(new javax.swing.JFrame(), true);
