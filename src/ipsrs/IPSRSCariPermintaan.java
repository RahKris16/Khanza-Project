...
public class IPSRSCariPermintaan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        ppCetakNonMedis = new javax.swing.JMenuItem();
        ...
        ppCetakNonMedis.setBackground(new java.awt.Color(255, 255, 254));
        ppCetakNonMedis.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppCetakNonMedis.setForeground(new java.awt.Color(50, 50, 50));
        ppCetakNonMedis.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppCetakNonMedis.setText("Cetak Permintaan Non Medis");
        ppCetakNonMedis.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppCetakNonMedis.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppCetakNonMedis.setName("ppCetakNonMedis"); // NOI18N
        ppCetakNonMedis.setPreferredSize(new java.awt.Dimension(225, 25));
        ppCetakNonMedis.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppCetakNonMedisActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppCetakNonMedis);
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    ...
    private void ppCetakNonMedisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppCetakNonMedisActionPerformed
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
        }else{
            if(tbDokter.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.queryu("delete from temporary_ipsrs");
                try {
                    i=0;
                    ps=koneksi.prepareStatement(
                        "select ipsrsbarang.nama_brng,detail_permintaan_non_medis.jumlah,kodesatuan.satuan,detail_permintaan_non_medis.keterangan "
                        + "from permintaan_non_medis INNER JOIN detail_permintaan_non_medis on detail_permintaan_non_medis.no_permintaan=permintaan_non_medis.no_permintaan "
                        + "INNER JOIN ipsrsbarang ON ipsrsbarang.kode_brng=detail_permintaan_non_medis.kode_brng "
                        + "INNER JOIN kodesatuan ON kodesatuan.kode_sat=detail_permintaan_non_medis.kode_sat "
                        + "where permintaan_non_medis.no_permintaan=?");
                    try {
                        ps.setString(1,tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString());
                        rs=ps.executeQuery();
                        while(rs.next()){
                            Sequel.menyimpan("temporary_ipsrs","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                ...
                param.put("kontakrs",akses.getkontakrs());
                param.put("propinsirs",akses.getpropinsirs());
                param.put("tanggal",Sequel.cariIsi("select DATE_FORMAT(permintaan_non_medis.tanggal,'%d-%m-%Y') as tanggal from permintaan_non_medis where permintaan_non_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                param.put("nopermintaan",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString());
                param.put("bangsal",Sequel.cariIsi("select ruang from permintaan_non_medis where permintaan_non_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                param.put("petugas",Sequel.cariIsi("select pegawai.nama from permintaan_non_medis inner join pegawai on pegawai.nik=permintaan_non_medis.nip where permintaan_non_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                ...
                Valid.MyReportqry("rptPermintaanIPSRS.jasper","report","::[ Lembar Permintaan Non Medis ]::","select * from temporary_ipsrs order by temporary_ipsrs.no",param);
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
    }//GEN-LAST:event_ppCetakNonMedisActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem ppCetakNonMedis;
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    ps2=koneksi.prepareStatement(
                                ...
                                "detail_permintaan_non_medis.jumlah,detail_permintaan_non_medis.keterangan,ipsrsbarang.harga "+
