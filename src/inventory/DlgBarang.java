...
public class DlgBarang extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        DTPExpired.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "24-02-2025" }));
        ...
        Status.setForeground(new java.awt.Color(50, 70, 50));
        Status.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "BPJS", "-" }));
        Status.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1));
        Status.setName("Status"); // NOI18N
        FormInput.add(Status);
        Status.setBounds(865, 10, 100, 22);

        label13.setText("Status :");
        label13.setName("label13"); // NOI18N
        label13.setPreferredSize(new java.awt.Dimension(65, 23));
        FormInput.add(label13);
        label13.setBounds(765, 12, 88, 23);
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (Kd.getText().trim().equals("")) {
            ...
            if(Valid.editTabletf(tabMode, "databarang", "kode_brng", "?", "nama_brng=?,kode_brng=?,kapasitas=?,kode_sat=?,letak_barang=?,"
                ...
                if(Sequel.cariInteger("select count(databarang_tambahan.kode_brng) from databarang_tambahan where databarang_tambahan.kode_brng=?",Kd.getText())>0){
                    Valid.editTable(tabMode,"databarang_tambahan","kode_brng",Kd,"status='"+Status.getSelectedItem().toString()+"'");
                }else {
                    Sequel.menyimpan("databarang_tambahan","'"+Kd.getText()+"','"+Status.getSelectedItem().toString()+"'");
                }
                emptTeks();
            }
        }
...
private void KapasitasKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KapasitasKeyPressed
            ...
            try {
                if(TCari.getText().trim().equals("")){
                    ps = koneksi.prepareStatement(
                            ...
                            + " databarang.karyawan,databarang.stokminimal, databarang.kdjns,"
                            + " jenis.nama,kapasitas,databarang.expire,databarang.kode_industri,industrifarmasi.nama_industri, "
                            + " databarang.kode_kategori,kategori_barang.nama as kategori,databarang.kode_golongan,golongan_barang.nama as golongan,dt.status "
                            + " from databarang inner join kodesatuan on databarang.kode_sat=kodesatuan.kode_sat "
                            + " inner join kodesatuan as satuanbesar on databarang.kode_satbesar=satuanbesar.kode_sat "
                            + " inner join jenis on databarang.kdjns=jenis.kdjns "
                            + " inner join industrifarmasi on databarang.kode_industri=industrifarmasi.kode_industri "
                            + " inner join golongan_barang on databarang.kode_golongan=golongan_barang.kode "
                            + " inner join kategori_barang on databarang.kode_kategori=kategori_barang.kode "
                            + " left join databarang_tambahan dt on dt.kode_brng =databarang.kode_brng "
    ...
    private void getData() {
        ...
        if (row != -1) {
            ...
            Status.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),32).toString());
