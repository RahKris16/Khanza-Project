...
public class DlgCariPermintaan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        ppCetakObatBhp = new javax.swing.JMenuItem();
        ...
        ppCetakObatBhp.setBackground(new java.awt.Color(255, 255, 254));
        ppCetakObatBhp.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppCetakObatBhp.setForeground(new java.awt.Color(50, 50, 50));
        ppCetakObatBhp.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppCetakObatBhp.setText("Cetak Permintaan Obat");
        ppCetakObatBhp.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppCetakObatBhp.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppCetakObatBhp.setName("ppCetakObatBhp"); // NOI18N
        ppCetakObatBhp.setPreferredSize(new java.awt.Dimension(225, 25));
        ppCetakObatBhp.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppCetakObatBhpActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppCetakObatBhp);
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    ...
    private void ppCetakObatBhpActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppCetakObatBhpActionPerformed
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
        }else{
            if(tbDokter.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.queryu("delete from temporary_permintaan_obatbhp");
                try {
                    i=0;
                    ps=koneksi.prepareStatement(
                        "select databarang.nama_brng,detail_permintaan_medis.jumlah,kodesatuan.satuan,detail_permintaan_medis.keterangan "
                        + "from permintaan_medis INNER JOIN detail_permintaan_medis on detail_permintaan_medis.no_permintaan=permintaan_medis.no_permintaan "
                        + "INNER JOIN databarang ON databarang.kode_brng=detail_permintaan_medis.kode_brng "
                        + "INNER JOIN kodesatuan ON kodesatuan.kode_sat=detail_permintaan_medis.kode_sat "
                        + "where permintaan_medis.no_permintaan=?");
                    try {
                        ps.setString(1,tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString());
                        rs=ps.executeQuery();
                        while(rs.next()){
                            Sequel.menyimpan("temporary_permintaan_obatbhp","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                                ""+i,rs.getString("nama_brng"),rs.getString("jumlah"),rs.getString("satuan"),rs.getString("keterangan"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
                            });
                            i++;
                        }
                    } catch (Exception e) {
                ...
                param.put("kontakrs",akses.getkontakrs());
                param.put("propinsirs",akses.getpropinsirs());
                param.put("tanggal",Sequel.cariIsi("select DATE_FORMAT(permintaan_medis.tanggal,'%d-%m-%Y') as tanggal from permintaan_medis where permintaan_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                param.put("nopermintaan",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString());
                param.put("bangsal",Sequel.cariIsi("select bangsal.nm_bangsal from permintaan_medis inner join bangsal on bangsal.kd_bangsal=permintaan_medis.kd_bangsal where permintaan_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                param.put("petugas",Sequel.cariIsi("select pegawai.nama from permintaan_medis inner join pegawai on pegawai.nik=permintaan_medis.nip where permintaan_medis.no_permintaan=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                ...
                Valid.MyReportqry("rptPermintaanMedis.jasper","report","::[ Lembar Permintaan Obat/BHP ]::","select * from temporary_permintaan_obatbhp order by temporary_permintaan_obatbhp.no",param);
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
    }//GEN-LAST:event_ppCetakObatBhpActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem ppCetakObatBhp;
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    ps2=koneksi.prepareStatement(
                            ...
                            "detail_permintaan_medis.jumlah,detail_permintaan_medis.keterangan,databarang.ralan "+
                    ...
                    try {
                        ...
                        int no=1;
                        while(rs2.next()){
                            total=rs2.getDouble("jumlah")*rs2.getDouble("ralan");
                            ttl=ttl+total;
                            tabMode.addRow(new Object[]{
                                "",rs2.getString("kode_brng")+" "+rs2.getString("nama_brng"),
                                rs2.getString("jumlah"),rs2.getString("satuan"),Valid.SetAngka(total)
                            });
                        } 
                    } catch (Exception e) {
