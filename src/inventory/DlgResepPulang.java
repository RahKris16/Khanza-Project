    ...
    private void ppResepObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppResepObatActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(Sequel.cariInteger(
                    "select count(*) from resep_pulang where resep_pulang.no_rawat=?",TNoRw.getText())>0){
                param.put("logo",Sequel.cariGambar("select setting.logo from setting")); 
                Valid.MyReportqry("rptItemResepPulang.jasper","report","::[ Aturan Pakai Obat ]::",
                    ...
                    "where resep_pulang.no_rawat='"+TNoRw.getText()+"' and resep_pulang.tanggal='"+tbResep.getValueAt(tbResep.getSelectedRow(),1).toString()+"' and resep_pulang.jam='"+tbResep.getValueAt(tbResep.getSelectedRow(),2).toString()+"'",param);
            }  
            this.setCursor(Cursor.getDefaultCursor());
        }
