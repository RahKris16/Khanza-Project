...
public class DlgCariPenjualan extends javax.swing.JDialog {
    ...
    public DlgCariPenjualan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"No.Nota","Tanggal","Petugas","Pasien","Keterangan","Jns.Jual","PPN Obat","Asal Stok","","","","Ongkos Kirim","Akun Bayar","Tgl Verif","Jam Verif"};
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    ...
    */
    ...
    private void tampil() {
        tanggal="  p.tgl_jual between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' ";
        nofak="";mem="";ptg="";sat="";bar="";statusbayar="";datacari="";
        if(!NoNota.getText().equals("")){
            nofak=" and p.nota_jual='"+NoNota.getText()+"' ";
        }        
        if(!nmmem.getText().equals("")){
            mem=" and p.nm_pasien='"+nmmem.getText()+"' ";
        }
        if(!nmptg.getText().equals("")){
            ptg=" and pg.nama='"+nmptg.getText()+"' ";
        }
        if(!nmsat.getText().equals("")){
            sat=" and j.nama='"+nmsat.getText()+"' ";
        }
        if(!nmbar.getText().equals("")){
            bar=" and db.nama_brng='"+nmbar.getText()+"' ";
        }
        if(!Status.getSelectedItem().toString().equals("Semua")){
            statusbayar=" and p.status='"+Status.getSelectedItem().toString()+"' ";
        }
        if(!TCari.getText().trim().equals("")){
            datacari=" and (p.nota_jual like '%"+TCari.getText()+"%' or p.no_rkm_medis like '%"+TCari.getText()+"%' or "+
                    "p.nm_pasien like '%"+TCari.getText()+"%' or  p.nip like '%"+TCari.getText()+"%' or "+
                    "pg.nama like '%"+TCari.getText()+"%' or p.keterangan like '%"+TCari.getText()+"%' or "+
                    "p.jns_jual like '%"+TCari.getText()+"%' or dj.kode_brng like '%"+TCari.getText()+"%' or "+
                    "b.nm_bangsal like '%"+TCari.getText()+"%' or db.nama_brng like '%"+TCari.getText()+"%' or "+
                    "dj.kode_sat like '%"+TCari.getText()+"%' or j.nama like '%"+TCari.getText()+"%') ";
        }

        Valid.tabelKosong(tabMode);
        try{
            ttljual=0;
            ps=koneksi.prepareStatement("SELECT p.nota_jual,DATE_FORMAT(p.tgl_jual, '%d-%m-%Y') AS tgl_jual,p.nip,pg.nama,p.no_rkm_medis,\n" +
                    "p.nm_pasien,p.keterangan,p.jns_jual,p.ongkir,b.nm_bangsal,p.status,p.ppn,p.nama_bayar,\n" +
                    "IFNULL(DATE_FORMAT(pw.tanggal, '%d-%m-%Y'), '') AS tgl_verif,IFNULL(TIME(pw.tanggal), '') AS jam_verif\n" +
                    "FROM penjualan p\n" +
                    "INNER JOIN petugas pg ON p.nip = pg.nip\n" +
                    "INNER JOIN bangsal b ON p.kd_bangsal = b.kd_bangsal\n" +
                    "INNER JOIN detailjual dj ON p.nota_jual = dj.nota_jual\n" +
                    "INNER JOIN databarang db ON dj.kode_brng = db.kode_brng\n" +
                    "INNER JOIN kodesatuan ks ON dj.kode_sat = ks.kode_sat\n" +
                    "INNER JOIN jenis j ON db.kdjns = j.kdjns\n" +
                    "LEFT JOIN penjualan_waktu pw ON pw.no_nota = p.nota_jual "+
                    " where "+tanggal+nofak+mem+ptg+sat+bar+statusbayar+datacari+
                    "GROUP BY p.nota_jual ORDER BY p.tgl_jual, p.nota_jual");
            try {
                rs=ps.executeQuery();
                ttlsubttl=0;
                ttlppn=0;
                ...
                while(rs.next()){                    
                    tabMode.addRow(new String[]{
                        ...
                        df2.format(rs.getDouble(12)),rs.getString(10),"","","",df2.format(rs.getDouble(9)),
