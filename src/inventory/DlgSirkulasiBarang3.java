...
public class DlgSirkulasiBarang3 extends javax.swing.JDialog {
    ...
    private double ttltotaljual=0,totaljual=0,jumlahjual=0,ttltotalbeli=0,totalbeli=0,jumlahbeli=0,
                   ...
                   ttltotalmutasimasuk=0,ttltotalmutasikeluar=0,jumlahhibah=0,totalhibah=0,ttltotalhibah=0,ttlkeluar=0,
                   ttlselisih=0,stokharian=0;
    ...
    public DlgSirkulasiBarang3(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Kode Barang","Nama Barang","Satuan","Stok","Keluar","Selisih","Stok /Hari","Stok(Rp)","Pengadaan","Pengadaan(Rp)",
    ...
    private void prosesCari() {
       ...
       try{   
            ps=koneksi.prepareStatement(
                    ...
                    "from databarang left join kodesatuan on databarang.kode_sat=kodesatuan.kode_sat "+
                    ...
                    "left join kategori_barang on databarang.kode_kategori=kategori_barang.kode where "+
                    "jenis.nama like ? and kategori_barang.nama like ? and golongan_barang.nama like ? and "+
                    "(databarang.kode_brng like ? or databarang.nama_brng like ?) and databarang.status<>'0' order by databarang.kode_brng");
            try {
                ...
                while(rs.next()){
                    ...
                    ttlkeluar=jumlahpasin+jumlahkeluar+jumlahrespulang+jumlahjual;
                    ttlselisih=stok-ttlkeluar;
                    stokharian=stok/(ttlkeluar/14);
                    if(ChkJln1.isSelected()==false){
                       if((stok>-1)&&(aset>-1)){
                        tabMode.addRow(new Object[]{rs.getString(1),rs.getString(2),
                           rs.getString(3),Valid.SetAngka(stok),Valid.SetAngka(ttlkeluar),
                           Valid.SetAngka(ttlselisih),Valid.SetAngka(stokharian),
                           Valid.SetAngka(aset),
                    ...
                    }else if(ChkJln1.isSelected()==true){
                        if(((ttlselisih<1))){
                            tabMode.addRow(new Object[]{rs.getString(1),rs.getString(2),
                            rs.getString(3),Valid.SetAngka(stok),Valid.SetAngka(ttlkeluar),
                            Valid.SetAngka(ttlselisih),Valid.SetAngka(stokharian),
