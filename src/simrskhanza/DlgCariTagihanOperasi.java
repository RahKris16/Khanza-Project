...
public class DlgCariTagihanOperasi extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jLabel10.setText("Prosedur Tindakan");
        ...
        panelGlass7.add(Prosedur);
        Prosedur.setBounds(30, 255, 256, 23);
                    ...
                    try {
                        try {
                            rs=koneksi.prepareStatement(
                                "SELECT ppo.no_rawat,ppo.tanggal,d.nm_dokter,ppo.ringkasan_klinik,ppo.pemeriksaan_fisik,ppo.pemeriksaan_diagnostik,ppo.diagnosa_pre_operasi,ppo.rencana_tindakan_bedah,ppo.hal_hal_yang_perludi_persiapkan,ppo.terapi_pre_operasi\n" +
                                "FROM penilaian_pre_operasi ppo \n" +
                                "inner join dokter d on d.kd_dokter=ppo.kd_dokter \n" +
                                "WHERE ppo.no_rawat ='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()+"'\n" +
                                "UNION\n" +
                                "SELECT '','','','','','','','','',''\n" +
                                "WHERE NOT EXISTS (\n" +
                                "    SELECT 1\n" +
...
//                                "FROM penilaian_pre_operasi ppo \n" +
//                                "inner join dokter d on d.kd_dokter=ppo.kd_dokter \n" +
//                                "WHERE ppo.no_rawat ='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()+"'").executeQuery();
                            if(rs.next()){
                                param.put("tanggal",rs.getString("tanggal"));
                                param.put("nm_dokter",rs.getString("nm_dokter"));
                                param.put("ringkasan_klinik",rs.getString("ringkasan_klinik"));
                                param.put("pemeriksaan_fisik",rs.getString("pemeriksaan_fisik"));
                                param.put("pemeriksaan_diagnostik",rs.getString("pemeriksaan_diagnostik"));
                                param.put("diagnosa_pre_operasi",rs.getString("diagnosa_pre_operasi"));
                                param.put("rencana_tindakan_bedah",rs.getString("rencana_tindakan_bedah"));
                                param.put("hal_hal_yang_perludi_persiapkan",rs.getString("hal_hal_yang_perludi_persiapkan"));
                                param.put("terapi_pre_operasi",rs.getString("terapi_pre_operasi"));
    ...
    private void MnUbahLaporanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnUbahLaporanActionPerformed
        if(tbDokter.getSelectedRow()>-1){
            if(!tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString().equals("")){
                try {
                    rs2=koneksi.prepareStatement(
                        "select diagnosa_preop, diagnosa_postop, jaringan_dieksekusi, selesaioperasi, permintaan_pa, laporan_operasi, prosedur_tindakan from laporan_operasi "+
                    ...
                    if(rs2.next()){
                        ...
                        Prosedur.setText(rs2.getString("prosedur_tindakan"));
                    }
                    if(rs2!=null){
                        rs2.close();
                    }
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(!Laporan.getText().equals("")){
            ...
            Sequel.menyimpan("laporan_operasi","?,?,?,?,?,?,?,?,?","laporan operasi",9,new String[]{
                ...
                DikirimPA.getSelectedItem().toString(),Laporan.getText(),Prosedur.getText()
            });
