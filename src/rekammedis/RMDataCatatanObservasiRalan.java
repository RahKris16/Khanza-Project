 ...
 * @author perpustakaan
 */
public final class RMDataCatatanObservasiRalan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public RMDataCatatanObservasiRalan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("catatan_observasi_ralan","?,?,?,?,?,?,?,?,?,?","Data",10,new String[]{
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(TCari.getText().trim().equals("")){
                Valid.MyReportqry("rptDataCatatanObservasiRanap.jasper","report","::[ Data Catatan Observasi Rawat Inap ]::",
                    ...
                    "pasien.jk,pasien.tgl_lahir,catatan_observasi_ralan.tgl_perawatan,catatan_observasi_ralan.jam_rawat,catatan_observasi_ralan.gcs,"+
                    "catatan_observasi_ralan.td,catatan_observasi_ralan.hr,catatan_observasi_ralan.rr,catatan_observasi_ralan.suhu,catatan_observasi_ralan.spo2,"+
                    "catatan_observasi_ralan.nip,petugas.nama from catatan_observasi_ralan inner join reg_periksa on catatan_observasi_ralan.no_rawat=reg_periksa.no_rawat "+
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_observasi_ralan.nip=petugas.nip where "+
                    "catatan_observasi_ralan.tgl_perawatan between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' order by catatan_observasi_ralan.tgl_perawatan",param);
            }else{
                Valid.MyReportqry("rptDataCatatanObservasiRanap.jasper","report","::[ Data Catatan Observasi Rawat Inap ]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"+
                    "pasien.jk,pasien.tgl_lahir,catatan_observasi_ralan.tgl_perawatan,catatan_observasi_ralan.jam_rawat,catatan_observasi_ralan.gcs,"+
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_observasi_ralan.nip=petugas.nip where "+
                    "catatan_observasi_ralan.tgl_perawatan between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' and "+
                    "(reg_periksa.no_rawat like '%"+TCari.getText().trim()+"%' or pasien.no_rkm_medis like '%"+TCari.getText().trim()+"%' or "+
                    "pasien.nm_pasien like '%"+TCari.getText().trim()+"%' or catatan_observasi_ralan.nip like '%"+TCari.getText().trim()+"%' or petugas.nama like '%"+TCari.getText().trim()+"%') "+
                    "order by catatan_observasi_ralan.tgl_perawatan ",param);
            }  
        }
    ...
    private void MnCatatanObservasiIGDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanObservasiIGDActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportqry("rptFormulirCatatanObservasiRanap.jasper","report","::[ Formulir Catatan Observasi Rawat Inap ]::",
                    ...
                    "pasien.jk,pasien.tgl_lahir,catatan_observasi_ralan.tgl_perawatan,catatan_observasi_ralan.jam_rawat,catatan_observasi_ralan.gcs,"+
                    "catatan_observasi_ralan.td,catatan_observasi_ralan.hr,catatan_observasi_ralan.rr,catatan_observasi_ralan.suhu,catatan_observasi_ralan.spo2,"+
                    "petugas.nama from catatan_observasi_ralan inner join reg_periksa on catatan_observasi_ralan.no_rawat=reg_periksa.no_rawat "+
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_observasi_ralan.nip=petugas.nip where reg_periksa.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"'",param);
        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanObservasiRalan dialog = new RMDataCatatanObservasiRalan(new javax.swing.JFrame(), true);
        ...
        try{
            if(TCari.getText().toString().trim().equals("")){
                ps=koneksi.prepareStatement(
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_observasi_ralan.nip=petugas.nip where "+
                    "catatan_observasi_ralan.tgl_perawatan between ? and ? order by catatan_observasi_ralan.tgl_perawatan");
            }else{
                ps=koneksi.prepareStatement(
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"+
                    "pasien.jk,pasien.tgl_lahir,catatan_observasi_ralan.tgl_perawatan,catatan_observasi_ralan.jam_rawat,catatan_observasi_ralan.gcs,"+
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_observasi_ralan.nip=petugas.nip where "+
                    "catatan_observasi_ralan.tgl_perawatan between ? and ? and (reg_periksa.no_rawat like ? or pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or catatan_observasi_ralan.nip like ? or petugas.nama like ?) "+
                    "order by catatan_observasi_ralan.tgl_perawatan ");
            }
    ...
    private void ganti() {
        Sequel.mengedit("catatan_observasi_ralan","tgl_perawatan=? and jam_rawat=? and no_rawat=?","no_rawat=?,tgl_perawatan=?,jam_rawat=?,gcs=?,td=?,hr=?,rr=?,suhu=?,spo2=?,nip=?",13,new String[]{
            TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),Jam.getSelectedItem()+":"+Menit.getSelectedItem()+":"+Detik.getSelectedItem(),
    ...
    private void hapus() {
        if(Sequel.queryu2tf("delete from catatan_observasi_ralan where tgl_perawatan=? and jam_rawat=? and no_rawat=?",3,new String[]{
