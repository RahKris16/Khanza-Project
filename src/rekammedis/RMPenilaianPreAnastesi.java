    ...
    public void setNoRm(String norwt,Date tgl2) {
        ...
        try {
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    Diagnosa.setText(rs.getString("diagnosis"));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
        ...
        try {
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    RencanaTindakan.setText(rs.getString("tindakan"));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
        ...
        try {
            ps=koneksi.prepareStatement(
                    "select pemeriksaan_ranap.tinggi,pemeriksaan_ranap.berat,pemeriksaan_ranap.tensi,pemeriksaan_ranap.spo2,"
                    + "pemeriksaan_ranap.nadi,pemeriksaan_ranap.suhu_tubuh,pemeriksaan_ranap.respirasi from pemeriksaan_ranap "
                    + "inner join dokter on dokter.kd_dokter=pemeriksaan_ranap.nip inner join spesialis on spesialis.kd_sps=dokter.kd_sps "
                    + "where pemeriksaan_ranap.no_rawat=? and spesialis.nm_sps='Anestesi' order by pemeriksaan_ranap.tgl_perawatan desc limit 1");
            try {
                ps.setString(1,norwt);
                rs=ps.executeQuery();
                while(rs.next()){
                    ...
                    IO2.setText(rs.getString("spo2"));
                    Nadi.setText(rs.getString("nadi"));
                    Suhu.setText(rs.getString("suhu_tubuh"));
                    Pernapasan.setText(rs.getString("respirasi"));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
