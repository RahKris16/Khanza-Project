...
public final class RMDataResumePasien extends javax.swing.JDialog {
    ...
    private String tanggal="",finger="",variabel="",ruangrujuk="";
        ...
        cariobat.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if(cariobat.getTable().getSelectedRow()!= -1){
                    Obat2an.append(cariobat.getTable().getValueAt(cariobat.getTable().getSelectedRow(),1).toString()+", ");
    ...
    private void initComponents() {
        ...
        BtnProtokol.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/inventaris.png"))); // NOI18N
        BtnProtokol.setMnemonic('T');
        BtnProtokol.setText("Protokol");
        BtnProtokol.setToolTipText("Alt+T");
        BtnProtokol.setName("BtnProtokol"); // NOI18N
        BtnProtokol.setPreferredSize(new java.awt.Dimension(100, 30));
        BtnProtokol.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnProtokolActionPerformed(evt);
            }
        });
        BtnProtokol.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                BtnProtokolKeyPressed(evt);
            }
        });
        panelGlass8.add(BtnProtokol);
        ...
        BtnDokter7.setBounds(180, 150, 28, 23);
            ...
            Map<String, Object> param = new HashMap<>(); 
                ...
                if(TCari.getText().equals("")){
                    Valid.MyReportqry("rptDataResumePasien.jasper","report","::[ Data Resume Pasien ]::",
                        ...
                        "resume_pasien.obat_pulang,bridging_surat_kontrol_bpjs_tambahan.status from resume_pasien inner join reg_periksa on resume_pasien.no_rawat=reg_periksa.no_rawat  "+
...
//        Valid.pindah(evt,TCari,Kondisi);
    ...
    private void MnLaporanResumeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanResumeActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            tanggal="";
            ruangrujuk=Sequel.cariIsi("select rujukan_internal_poli.kd_dokter from rujukan_internal_poli where rujukan_internal_poli.no_rawat=?",TNoRw.getText());
            if(Sequel.cariIsi("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat=?",TNoRw.getText()).equals("Ralan")){
                if(ruangrujuk.equals(tbObat.getValueAt(tbObat.getSelectedRow(),4).toString())){
    ...
    private void BtnDokter6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnDokter6ActionPerformed
        if(TNoRw.getText().equals("")&&TNoRM.getText().equals("")){
            ...
            caripemeriksaan2.setNoRawat(TNoRw.getText());
            caripemeriksaan2.tampil();
            caripemeriksaan2.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            caripemeriksaan2.setLocationRelativeTo(internalFrame1);
            caripemeriksaan2.setVisible(true);
        }
    ...
    private void BtnDokter7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnDokter7ActionPerformed
        if(TNoRw.getText().equals("")&&TNoRM.getText().equals("")){
            ...
            caripemeriksaan3.setNoRawat(TNoRw.getText());
            caripemeriksaan3.tampil();
            caripemeriksaan3.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
            caripemeriksaan3.setLocationRelativeTo(internalFrame1);
            caripemeriksaan3.setVisible(true);
        }
    ...
    private void BtnProtokolActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnProtokolActionPerformed
        ...
        protokol.setNoRm3(TNoRw.getText());
        protokol.tampil();
        protokol.setVisible(true);
        this.setCursor(Cursor.getDefaultCursor());
    }//GEN-LAST:event_BtnProtokolActionPerformed

    private void BtnProtokolKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnProtokolKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_BtnProtokolKeyPressed
    ...
    */
    ...
    private widget.Button BtnProtokol;
...
//        }
        ...
        try {
            ps=koneksi.prepareStatement(
                "SELECT\n" +
                "TRIM(SUBSTRING_INDEX(tindakan, ',', 1)) AS bagian1,\n" +
                "IF(\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 2), ',', -1))\n" +
                "    = TRIM(SUBSTRING_INDEX(tindakan, ',', 1)),\n" +
                "    '',\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 2), ',', -1))\n" +
                ") AS bagian2,\n" +
                "IF(\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 3), ',', -1))\n" +
                "    = TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 2), ',', -1)),\n" +
                "    '',\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 3), ',', -1))\n" +
                ") AS bagian3,\n" +
                "IF(\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 4), ',', -1))\n" +
                "    = TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 3), ',', -1)),\n" +
                "    '',\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 4), ',', -1))\n" +
                ") AS bagian4,\n" +
                "IF(\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 5), ',', -1))\n" +
                "    = TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 4), ',', -1)),\n" +
                "    '',\n" +
                "    TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(tindakan, ',', 5), ',', -1))\n" +
                ") AS bagian5\n" +
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    ProsedurUtama.setText(rs.getString("bagian1"));
                    ProsedurSekunder1.setText(rs.getString("bagian2"));
                    ProsedurSekunder2.setText(rs.getString("bagian3"));
                    ProsedurSekunder3.setText(rs.getString("bagian4"));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
        ...
        try {
            ps=koneksi.prepareStatement(
                    //"select pemeriksaan_ralan.suhu_tubuh,pemeriksaan_ralan.tensi,pemeriksaan_ralan.nadi,pemeriksaan_ralan.respirasi,pemeriksaan_ralan.gcs "+
                    "select pemeriksaan_ralan.pemeriksaan "+
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    JalannyaPenyakit.setText(rs.getString(1));
                    //JalannyaPenyakit.setText("Suhu: "+rs.getString(1)+" °C, TD: "+rs.getString(2)+", Nadi: "+rs.getString(3)+"X/menit, RR: "+rs.getString(4)+"X/menit, GCS: "+rs.getString(5));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
        ...
        try {
            ps=koneksi.prepareStatement(
                    "SELECT GROUP_CONCAT(' ',databarang.nama_brng,' ',resep_dokter.aturan_pakai SEPARATOR ';') as resume FROM resep_dokter "+
                    ...
                    "WHERE resep_obat.no_rawat=? AND resep_dokter.aturan_pakai<>''");
            try {
                ps.setString(1,norwt);
                rs=ps.executeQuery();
                while(rs.next()){
                    Obat2an.setText(rs.getString("resume"));
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            } finally{
