...
public final class RMPenilaianAwalMedisRalanDewasa extends javax.swing.JDialog {
    ...
    public RMPenilaianAwalMedisRalanDewasa(java.awt.Frame parent, boolean modal) {
        ...
        diagnosa1.addWindowListener(new WindowListener() {
            ...
            @Override
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("RMPenilaianAwalMedisRalanDewasa")){
                    if(diagnosa1.getTable().getSelectedRow()!= -1){                   
                        ...
                        diagnosa.setText(diagnosa1.getTable().getValueAt(diagnosa1.getTable().getSelectedRow(),0).toString());
                    }  
    ...
    private void initComponents() {
        ...
        diagnosa.setBounds(2000, 990, 45, 23);
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "11-12-2023 12:06:38" }));
        ...
        jLabel104.setText("VII. KONSUL/RUJUK/KONTROL");
        ...
        btnDiagnosa1.setBounds(860, 990, 28, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRM.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("penilaian_medis_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",35,new String[]{
                    ...
                    Sequel.menyimpantf("pemeriksaan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",21,new String[]{
                    TNoRw.getText(),Valid.SetTgl(TglAsuhan.getSelectedItem()+""),TglAsuhan.getSelectedItem().toString().substring(11,19),Suhu.getText(),TD.getText(),Nadi.getText(),RR.getText(),TB.getText(),BB.getText(),
                    SPO.getText(),GCS.getText(),Kesadaran.getSelectedItem().toString(),KeluhanUtama.getText(),KetFisik.getText(),Alergi.getText(),"","",Diagnosis.getText(),"","",KdDokter.getText()
                    });
                    status="Ralan";
                    if(Sequel.cariInteger(
                            ...
                            "pasien.no_rkm_medis='"+TNoRM.getText()+"' and diagnosa_pasien.kd_penyakit='"+diagnosa.getText()+"'")>0){
                        Sequel.menyimpan("diagnosa_pasien","?,?,?,?,?","Penyakit",5,new String[]{
                            TNoRw.getText(),diagnosa.getText(),status,
                            Sequel.cariIsi("select ifnull(MAX(diagnosa_pasien.prioritas)+1,1) from diagnosa_pasien where diagnosa_pasien.no_rawat=? and diagnosa_pasien.status='"+status+"'",TNoRw.getText()),"Lama"
                        });
                    }else{
                        Sequel.menyimpan("diagnosa_pasien","?,?,?,?,?","Penyakit",5,new String[]{
                            TNoRw.getText(),diagnosa.getText(),status,
                            Sequel.cariIsi("select ifnull(MAX(diagnosa_pasien.prioritas)+1,1) from diagnosa_pasien where diagnosa_pasien.no_rawat=? and diagnosa_pasien.status='"+status+"'",TNoRw.getText()),"Baru"
                        });
                    }
                    emptTeks();
            }
    ...
    private void btnDiagnosa1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDiagnosa1ActionPerformed
        akses.setform("RMPenilaianAwalMedisRalanDewasa");
        diagnosa1.isCek();
