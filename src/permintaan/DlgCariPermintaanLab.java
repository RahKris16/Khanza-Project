...
public class DlgCariPermintaanLab extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "17-03-2024 21:33:05" }));
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(tbLabRalan.getSelectedRow()!= -1){
                ...
                try {
                    i=0;
                    ps=koneksi.prepareStatement(
                        "SELECT permintaan_lab.no_rawat,permintaan_lab.status,jns_perawatan_lab.nm_perawatan FROM permintaan_lab "
                        ...
                        + "WHERE permintaan_lab.noorder=? GROUP BY permintaan_detail_permintaan_lab.kd_jenis_prw");
                    try {
                        ps.setString(1,NoPermintaan);
                        rs=ps.executeQuery();
                        while(rs.next()){
                ...
                param.put("diagnosa",Sequel.cariIsi("select diagnosa_klinis from permintaan_lab where noorder=?",NoPermintaan));
                ...
                param.put("tanggal",Sequel.cariIsi("select DATE_FORMAT(permintaan_lab.tgl_permintaan,'%d-%m-%Y') as tanggal from permintaan_lab where permintaan_lab.noorder=?",NoPermintaan));
        ...
        try {
            ...
            if(cmbStatus.getSelectedIndex()==0){
                ps=koneksi.prepareStatement(
                    ...
                    "inner join penjab on reg_periksa.kd_pj=penjab.kd_pj "+
                    "where permintaan_lab.status='ranap' and (kamar_inap.stts_pulang='-' or kamar_inap.stts_pulang<>'Pindah Kamar') and permintaan_lab.tgl_permintaan between ? and ? "+
