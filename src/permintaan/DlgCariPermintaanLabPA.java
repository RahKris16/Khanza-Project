...
public class DlgCariPermintaanLabPA extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "17-03-2024 21:55:12" }));
        ...
        ppLembarObat1.setText("Lembar Order Lab");
        ...
        ppLembarObat2.setText("Lembar Order Lab");
        ...
        MnBarcodeRM1.setText("Label Rekam Medis");
    ...
    private void ppLembarObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObat1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbLabRalan.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.queryu("delete from temporary_lab2");
                try {
                    i=0;
                    ps=koneksi.prepareStatement(
                        "SELECT permintaan_labpa.no_rawat,permintaan_labpa.status,jns_perawatan_lab.nm_perawatan FROM permintaan_labpa "
                        + "INNER JOIN permintaan_pemeriksaan_labpa on permintaan_pemeriksaan_labpa.noorder=permintaan_labpa.noorder "
                        + "INNER JOIN jns_perawatan_lab on jns_perawatan_lab.kd_jenis_prw=permintaan_pemeriksaan_labpa.kd_jenis_prw "
                        + "WHERE permintaan_labpa.noorder=? GROUP BY permintaan_pemeriksaan_labpa.kd_jenis_prw");
                    try {
                        ps.setString(1,NoPermintaan);
                        rs=ps.executeQuery();
                        while(rs.next()){
                            Sequel.menyimpan("temporary_lab2","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                                ""+i,rs.getString("no_rawat"),rs.getString("status"),rs.getString("nm_perawatan"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
                            });
                            i++;
                        }
                    } catch (Exception e) {
                ...
                param.put("diagnosa",Sequel.cariIsi("select diagnosa_klinis from permintaan_labpa where noorder=?",NoPermintaan));
                param.put("jk",Sequel.cariIsi("select pasien.jk from pasien where pasien.no_rkm_medis=?",Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=?",NoRawat)));
                ...
                param.put("tanggal",Sequel.cariIsi("select DATE_FORMAT(permintaan_labpa.tgl_permintaan,'%d-%m-%Y') as tanggal from permintaan_labpa where permintaan_labpa.noorder=?",NoPermintaan));
                param.put("norawat",NoRawat);
                param.put("pasien",Pasien);
                param.put("perujuk",DokterPerujuk);
                ...
                Valid.MyReportqry("rptLembarLab.jasper","report","::[ Lembar Order Lab ]::","select * from temporary_lab2 order by temporary_lab2.no",param);
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
        ...
        try {
            ...
            if(cmbStatus.getSelectedIndex()==0){
                ps=koneksi.prepareStatement(
                    ...
                    "inner join penjab on reg_periksa.kd_pj=penjab.kd_pj "+
                    "where permintaan_labpa.status='ranap' and (kamar_inap.stts_pulang='-' or kamar_inap.stts_pulang<>'Pindah Kamar') and permintaan_labpa.tgl_permintaan between ? and ? "+
