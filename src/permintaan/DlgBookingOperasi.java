...
public class DlgBookingOperasi extends javax.swing.JDialog {
    ...
    private int i=0;
    private String status="",bangsal="",lokasistok="",kamar="",diagnosa="",order="",kelas="",penjab="",norawatibu="",posisi="",cbayar="",DEPOAKTIFOBAT="",norm="";
        ...
        if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                ...
                if(Sequel.menyimpantf("booking_operasi","?,?,?,?,?,?,?,?","data", 8,new String[]{
                    ...
                    Sequel.menyimpan("booking_operasi_poli","?,?","data", 2,new String[]{
    ...
    public void tampil() {     
        ...
        try {
            ps=koneksi.prepareStatement(
                    ...
                    "left join ruang_ok on booking_operasi.kd_ruang_ok=ruang_ok.kd_ruang_ok "+
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    if(kamar.equals("")){
                        if(Sequel.cariInteger("select ruang from booking_operasi_poli where no_rawat=? ",rs.getString("no_rawat"))>0)
                        {
                            kamar=Sequel.cariIsi("select ruang from booking_operasi_poli where no_rawat=? ",rs.getString("no_rawat"));
                        }else{
                    ...
                    diagnosa=Sequel.cariIsi("select concat(diagnosa_pasien.kd_penyakit,' ',penyakit.nm_penyakit) from diagnosa_pasien inner join penyakit on diagnosa_pasien.kd_penyakit=penyakit.kd_penyakit where diagnosa_pasien.no_rawat=? order by diagnosa_pasien.prioritas desc limit 1",rs.getString("no_rawat"));
