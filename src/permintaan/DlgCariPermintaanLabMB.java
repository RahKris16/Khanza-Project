...
public class DlgCariPermintaanLabMB extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "19-06-2024 11:34:21" }));
            ...
            if(tbLabRalan.getSelectedRow()!= -1){
                ...
                try {
                    ...
                    ps=koneksi.prepareStatement(
                        "SELECT permintaan_labmb.no_rawat,permintaan_labmb.status,jns_perawatan_lab.nm_perawatan FROM permintaan_labmb "
                        + "INNER JOIN permintaan_pemeriksaan_labmb on permintaan_pemeriksaan_labmb.noorder=permintaan_labmb.noorder "
                        + "INNER JOIN jns_perawatan_lab on jns_perawatan_lab.kd_jenis_prw=permintaan_pemeriksaan_labmb.kd_jenis_prw "
                        + "WHERE permintaan_labmb.noorder=? GROUP BY permintaan_pemeriksaan_labmb.kd_jenis_prw");
                    try {
                ...
                param.put("diagnosa",Sequel.cariIsi("select diagnosa_klinis from permintaan_labmb where noorder=?",NoPermintaan));
                ...
                param.put("tanggal",Sequel.cariIsi("select DATE_FORMAT(permintaan_labmb.tgl_permintaan,'%d-%m-%Y') as tanggal from permintaan_labmb where permintaan_labmb.noorder=?",NoPermintaan));
        ...
        try {
            ...
            if(cmbStatus.getSelectedIndex()==0){
                ps=koneksi.prepareStatement(
                    ...
                    "inner join penjab on reg_periksa.kd_pj=penjab.kd_pj "+
                    "where permintaan_labmb.status='ranap' and (kamar_inap.stts_pulang='-' or kamar_inap.stts_pulang<>'Pindah Kamar') and permintaan_labmb.tgl_permintaan between ? and ? "+
