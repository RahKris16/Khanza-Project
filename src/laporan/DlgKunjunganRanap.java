...
public final class DlgKunjunganRanap extends javax.swing.JDialog {
    ...
    private int i=0,lama=0,baru=0,laki=0,per=0,sep=0;  
    ...
    public DlgKunjunganRanap(java.awt.Frame parent, boolean modal) {
        ...
        setSize(885,674);
        tabMode=new DefaultTableModel(null,new String[]{"No.","Lama","Baru","Nama Pasien","L","P","Alamat","Cara Bayar","Kode","Diagnosa","Ruang","Stts.Pulang","Tgl.Masuk","Tgl.Keluar","DPJP"}){
    ...
    public void tampil(){        
        try{   
            ...
            ps=koneksi.prepareStatement(
                    "select reg_periksa.no_rawat,reg_periksa.tgl_registrasi as tgl_regis,reg_periksa.no_rkm_medis,concat(reg_periksa.no_rawat,' ',pasien.nm_pasien) as pasien,pasien.alamat,penjab.png_jawab as asuransi,pasien.jk,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur) as umur,pasien.tgl_daftar,reg_periksa.stts_daftar, kamar_inap.kd_kamar,bangsal.nm_bangsal,concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab)as almt_pj,kamar_inap.stts_pulang, "+
                    "DATE_FORMAT(reg_periksa.tgl_registrasi,'%d-%m-%Y') as tgl_masuk, DATE_FORMAT(kamar_inap.tgl_keluar,'%d-%m-%Y') as tgl_keluar,ifnull(dokter.nm_dokter,\"Belum ada DPJP\") as nm_dokter from reg_periksa "+
                    ...
                    "bangsal.nm_bangsal not like \"%KS%\" and reg_periksa.stts_daftar like '%"+status+"%' and reg_periksa.status_lanjut='Ranap' and reg_periksa.stts<>'Batal' and kamar_inap.stts_pulang<>'Pindah Kamar' and reg_periksa.tgl_registrasi between ? and ? and bangsal.nm_bangsal like ? and penjab.png_jawab like ? and kabupaten.nm_kab like ? and kecamatan.nm_kec like ? and kelurahan.nm_kel like ? and bangsal.nm_bangsal like ? "+
            ...
            try {
                ...
                rs=ps.executeQuery();
                i=1;   
                lama=0;baru=0;laki=0;per=0;sep=0;
                while(rs.next()){
                    try {
                        ps3=koneksi.prepareStatement("select count(no_rawat) as jumlah from bridging_sep where no_rawat=? ");
                        try {
                           ps3.setString(1,rs.getString("no_rawat"));
                           rs3=ps3.executeQuery();
                           while(rs3.next()){
                               sep=rs3.getInt("jumlah");
                           }
                        } catch (Exception e) {
                    ...
                    if(sep>0){
                        if(dokterdpjp.toLowerCase().contains(nmdokter.getText().toLowerCase())){
                            ...
                            tabMode.addRow(new Object[]{
                                i,setlama,setbaru,rs.getString("pasien"),umurlk,umurpr,rs.getString("almt_pj"),rs.getString("asuransi"),kddiagnosa,diagnosa,rs.getString("kd_kamar")+" "+rs.getString("nm_bangsal"),rs.getString("stts_pulang"),rs.getString("tgl_masuk"),rs.getString("tgl_keluar"),rs.getString("nm_dokter")
                            });                
                            i++;
                        }
                    }
