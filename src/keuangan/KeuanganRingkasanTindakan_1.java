 ...
 * @author perpustakaan
 */
public final class KeuanganRingkasanTindakan_1 extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public KeuanganRingkasanTindakan_1(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabModeRanapDokterParamedis=new DefaultTableModel(null,new Object[]{
            "No.","Kode","Perawatan/Tindakan","Jumlah"}){
             @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
             Class[] types = new Class[] {
    ...
    */
    ...
    public void tampil7(){     
        ...
        try{
                ps=koneksi.prepareStatement(
                    "SELECT ki.no_rawat,CONCAT(ki.tgl_masuk,' ',ki.jam_masuk) as awal,CONCAT(IF(ki.tgl_keluar = '0000-00-00', CURRENT_DATE(), ki.tgl_keluar),' ',IF(ki.jam_keluar = '00:00:00', CURRENT_TIME() , ki.jam_keluar)) as akhir,b.nm_bangsal \n" +
                    "FROM kamar_inap ki \n" +
                    "inner join kamar k on k.kd_kamar =ki.kd_kamar\n" +
                    "INNER JOIN bangsal b on b.kd_bangsal =k.kd_bangsal \n" +
                    "WHERE CONCAT(ki.tgl_masuk,' ',ki.jam_masuk) BETWEEN ? AND ? AND b.nm_bangsal LIKE ?");  
            try {
                ps.setString(1,Valid.SetTglJam(Tgl1.getSelectedItem()+""));
                ps.setString(2,Valid.SetTglJam(Tgl2.getSelectedItem()+""));
                ...
                while(rs.next()){
                    ps2=koneksi.prepareStatement(
                       "select jns_perawatan_inap.kd_jenis_prw,jns_perawatan_inap.nm_perawatan,count(jns_perawatan_inap.nm_perawatan) "+
                       "from jns_perawatan_inap inner join rawat_inap_drpr on rawat_inap_drpr.kd_jenis_prw=jns_perawatan_inap.kd_jenis_prw "+
                       "where concat(rawat_inap_drpr.tgl_perawatan,' ',rawat_inap_drpr.jam_rawat) between ? and ? group by jns_perawatan_inap.nm_perawatan order by jns_perawatan_inap.nm_perawatan asc");
                    try {
                        ps2.setString(1,rs.getString("awal"));
                        ps2.setString(2,rs.getString("akhir"));

                        rs2=ps2.executeQuery();
                        i=1;
                        item=0;
                        while(rs2.next()){
                            item=item+rs2.getInt(3);
                            tabModeRanapDokterParamedis.addRow(new Object[]{
                                i++,rs2.getString(1),rs2.getString(2),rs2.getInt(3)
                            });
                        }
                    } catch (Exception e) {
                        System.out.println("Notifikasi : "+e);
