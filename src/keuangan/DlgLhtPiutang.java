...
private void MnDetailCicilanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDetailCicilanActionPerformed
    ...
    */
    ...
    public void tampil(){
        ...
        try{
            sisapiutang=0;
            ps=koneksi.prepareStatement("select pp.no_rawat, date_format(pp.tgl_piutang, '%d-%m-%Y') as tgl_piutang, concat(pp.no_rkm_medis,' ',p2.nm_pasien),\n" +
                    "pp.status,pp.totalpiutang, pp.uangmuka, pp.sisapiutang, date_format(pp.tgltempo,'%d-%m-%Y') as tgltempo,p.png_jawab,\n" +
                    "ifnull(cp.catatan,\"\") as catatan,IF(rm.perujuk='-',\"\",ifnull(rm.perujuk,\"\")) as perujuk,rp.status_lanjut " +
                    "from piutang_pasien pp inner join pasien p2 on pp.no_rkm_medis=p2.no_rkm_medis \n" +
                    "inner join reg_periksa rp on pp.no_rawat=rp.no_rawat \n" +
                    "inner join penjab p on rp.kd_pj=p.kd_pj\n" +
                    "left join catatan_pasien cp on cp.no_rkm_medis=rp.no_rawat\n" +
                    "left join rujuk_masuk rm on rm.no_rawat=rp.no_rawat \n" +
                    "where concat(rp.kd_pj,p.png_jawab) like ? and pp.tgl_piutang between ? and ? and\n" +
                    "(pp.no_rawat like ? or pp.no_rkm_medis like ? or p2.nm_pasien like ? or pp.status like ?) order by pp.tgl_piutang");
            try {
                ...
                while(rs.next()){
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString(11),rs.getString(10),rs.getString("status_lanjut")
                    });
