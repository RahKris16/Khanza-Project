 ...
 * @author dosen
 */
public final class SatuSehatKirimAllergy extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public SatuSehatKirimAllergy(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new String[]{
                ...
                "Allergy","Petugas/Praktisi","No.KTP Praktisi","Tanggal","ID Allergy"
            }){
                    ...
                    try{
                        ...
                        headers.add("Authorization", "Bearer "+api.TokenSatuSehat());
                         ...
                         json = "{\n"
                                ...
                                + "            \"use\": \"official\",\n"
                                + "            \"value\": \"" + tbObat.getValueAt(i, 2).toString().replaceAll("/", "") + "\"\n"
                                + "        }\n"
                                + "    ],\n"
                                + "    \"clinicalStatus\": {\n"
                                + "        \"coding\": [\n"
                                ...
                                + "            }\n"
                                + "        ],\n"
                                + "        \"text\": \"" + tbObat.getValueAt(i, 7).toString() + "\"\n"
                                + "    },\n"
                                + "    \"patient\": {\n"
                                ...
                                + "    },\n"
                                + "    \"recordedDate\": \"" + tbObat.getValueAt(i, 10).toString() + "\",\n"
                                + "    \"recorder\": {\n"
                        ...
                        if(!response.asText().equals("")){
                            Sequel.menyimpan("satu_sehat_allergy","?,?","Allergy",2,new String[]{
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SatuSehatKirimAllergy dialog = new SatuSehatKirimAllergy(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
                   ...
                   " pegawai.nama,pegawai.no_ktp as ktppraktisi,DATE_FORMAT(concat(pemeriksaan_ralan.tgl_perawatan,' ',pemeriksaan_ralan.jam_rawat), '%Y-%m-%dT%H:%i:%s+00:00') AS tanggal,"+
                   "  ifnull(satu_sehat_allergy.id_allergy,'') as satu_sehat_allergy  "+
                   ...
                   "left join satu_sehat_allergy on satu_sehat_allergy.no_rawat=pemeriksaan_ralan.no_rawat  "+
                   
                   " where pemeriksaan_ralan.alergi<>'' and nota_jalan.tanggal between ? and ? "+
                   ...
                   "reg_periksa.no_rawat,pemeriksaan_ralan.tgl_perawatan");
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("tanggal"),rs.getString("satu_sehat_allergy")
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            ...
            ps=koneksi.prepareStatement(
                  "select reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,"+
                   ...
                   " pegawai.nama,pegawai.no_ktp as ktppraktisi,concat(pemeriksaan_ranap.tgl_perawatan,'T',pemeriksaan_ranap.jam_rawat) as tanggal,"+
                   "  ifnull(satu_sehat_allergy.id_allergy,'') as satu_sehat_allergy  "+
                   "  from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis  "+
                   ...
                   "left join satu_sehat_allergy on satu_sehat_allergy.no_rawat=pemeriksaan_ranap.no_rawat  "+
                   " and satu_sehat_allergy.tanggal=pemeriksaan_ranap.tgl_perawatan  "+
                   " where pemeriksaan_ranap.alergi<>'' and nota_inap.tanggal between ? and ? "+
                   (TCari.getText().equals("")?"":"and (reg_periksa.no_rawat like ? or reg_periksa.no_rkm_medis like ? or "+
                   "pasien.nm_pasien like ? or pasien.no_ktp like ? or pegawai.no_ktp like ? or pegawai.nama like ?) ")+
                   "order by reg_periksa.tgl_registrasi,reg_periksa.jam_reg,"+
                   "reg_periksa.no_rawat,pemeriksaan_ranap.tgl_perawatan");
            try {
                ps.setString(1,Valid.SetTgl(DTPCari1.getSelectedItem()+""));
                ps.setString(2,Valid.SetTgl(DTPCari2.getSelectedItem()+""));
                if(!TCari.getText().equals("")){
